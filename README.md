...
/*!< alink_read 缓冲数据的个数 */
CCFLAGS += -D CONFIG_DOWN_CMD_QUEUE_NUM=2

/*!< alink_write 缓冲数据的个数 */
CCFLAGS += -D CONFIG_UP_CMD_QUEUE_NUM=2

/*!< 事件处理函数的栈空间大小 */
CCFLAGS += -D CONFIG_EVENT_HANDLER_CB_STACK=256

/*!< 数据包的最大长度。模组认证时需要为1.5k，否则将无法通过超长数据包测试 */
CCFLAGS += -D CONFIG_ALINK_DATA_LEN=1536

/*!< 如果开发时内存空间不足可以参考如下配置，内存将增大3k左右 */
...
4. **数据通信**
    ...
    - 数据长度：默认长度为512KB，下行数据长度最大支持1k，如需修改请使用 Makefile
    - 数据上报：主动上报是由设备端主动发起
...
6. **信息存储**
    ...
    - 示例
        ...
        ```
    > 注：总存储信息量不能超过4KB，需占用12k的空间。为了保证在写 flash 时出现突然断电，对数据的破坏，我们采用了一个4k的空间对数据进行备份，另一个4k来记录存储的位置。
